...
type Runtime = "claude" | "codex" | "cursor";
...
const SUPPORTED_RUNTIMES: Runtime[] = ["claude", "codex", "cursor"];
...
function copyRecursive(src: string, dst: string, force: boolean, report: CopyReport): void {
...
function createFadAliases(targetDir: string): number {
...
function writeInstallAudit(targetDir: string, runtimes: Runtime[]): void {
  const payload = {
    tool: APP_NAME,
